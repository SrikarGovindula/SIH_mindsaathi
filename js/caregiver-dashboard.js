...
class CaregiverDashboard {
    ...
    detectClinicalAlerts() {
        // Titles are translated via i18n keys (alertHesitationTitle, etc.) so they
        // switch correctly for all 8 supported languages, not just Assamese/English.
        const t = (key, fallback) => (window.i18n ? window.i18n.t(key) : fallback);
        this.alerts = [
            {
                level: 'warning',
                icon: '⚠️',
                title: t('alertHesitationTitle', 'Hesitation Spike Observed'),
            ...
            },
            {
                level: 'success',
                icon: '🌟',
                title: t('alertReminiscenceTitle', 'High Reminiscence Engagement'),
            ...
            },
            {
                level: 'info',
                icon: '💧',
                title: t('alertHydrationTitle', 'Hydration Target On Track'),
    ...
    // Called on the 'languageChanged' event so the whole dashboard (domain
    // labels + alert titles) refreshes into the newly selected language.
    refreshLanguage() {
        this.detectClinicalAlerts();
        this.renderCognitiveRadar();
        this.renderAlerts();
    ...
    renderCognitiveRadar() {
        ...
        // Domain labels come from window.i18n so they switch with the selected
        // language instead of always showing hardcoded Assamese/Bengali text.
        const t = (key, fallback) => (window.i18n ? window.i18n.t(key) : fallback);
        const scores = this.patient.cognitiveScores;
        const domains = [
            { key: 'memory', label: t('memoryDomain', 'Memory'), score: scores.memory, color: '#27AE60', desc: 'Short-term visual & cultural recall' },
            { key: 'attention', label: t('attentionDomain', 'Attention'), score: scores.attention, color: '#2980B9', desc: 'Tea bud selective focus & reaction' },
            { key: 'routine', label: t('routineDomain', 'Routine'), score: scores.routine, color: '#8E44AD', desc: 'Chronological activity sequencing' },
            { key: 'executive', label: t('executiveDomain', 'Executive'), score: scores.executive, color: '#D35400', desc: 'Pattern deduction & problem solving' },
            { key: 'pattern', label: t('patternDomain', 'Visuospatial'), score: scores.pattern, color: '#C0392B', desc: 'Gamusa weave & fruit identification' }
        ];
